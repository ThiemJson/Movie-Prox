...
open class BaseViewController: UIViewController {
    ...
    open var interstitialAd                         : GADInterstitialAd?
    
    open var rewardedAd                             : GADRewardedAd?
    ...
    /** `UI Configuration` */
    open func setupUI() {
        self.view.backgroundColor       = UIColor.baseBackground
    }
    ...
    /** `Ads` */
    open func setupInterstitialAds(completion: @escaping (Bool) -> Void = { _ in }) {
        /** `Interstitial Ads` */
        let request = GADRequest()
        GADInterstitialAd.load(withAdUnitID: Constant.Ads.ads_interstitial,
                               request: request,
    ...
    open func setupRewardedAds(completion: @escaping (Bool) -> Void = { _ in }) {
        /** `Interstitial Ads` */
        let request = GADRequest()
        GADRewardedAd.load(withAdUnitID: Constant.Ads.ads_rewarded,
                           request: request,
                           completionHandler: { [weak self] (ad, error) in
            guard let `self` = self else { return }
            if let error = error {
    ...
    open func adsDidFailToPresentFullScreenContentWithError(_ ad: GADFullScreenPresentingAd,
                                                            _ didFailToPresentFullScreenContentWithError: Error) {}
    
    open func adsWillPresentFullScreenContent(_ ad: GADFullScreenPresentingAd) {}
    
    open func adWillDismissFullScreenContent(_ ad: GADFullScreenPresentingAd) {}
    
    open func adsDidDismissFullScreenContent(_ ad: GADFullScreenPresentingAd) {}
...
//                    if self == UIViewController.topViewController {
...
//                        internetErorVC.modalTransitionStyle     = .crossDissolve
//                        internetErorVC.modalPresentationStyle   = .overFullScreen
//                        self.present(internetErorVC, animated: true)
//                    }
                }
...
extension BaseViewController {
    public func showLoading(message: String? = nil, tintColor: UIColor = .white, style: UIActivityIndicatorView.Style = .whiteLarge) {
        DispatchQueue.main.async { [weak self] in
            guard let `self` = self else { return }
            self.showLoading(forView: self.view, message: message, tintColor: tintColor, style: style)
        }
    }
    
    public func showLoading(forView view: UIView?, message: String? = nil, tintColor: UIColor = .white, style: UIActivityIndicatorView.Style = .whiteLarge) {
        if let view = view {
            BaseLoading.shared.show(forView: view, message: message, style: style, tintColor: tintColor)
        }
    }
...
extension BaseViewController {
    ...
    func replaceRoot(to viewController: UIViewController,
                     ...
                     isWitNavbar isWithNavbar: Bool = true ) {
        
        let navVC       =  isWithNavbar ? viewController.embedInNavigationController() : viewController
...
extension BaseViewController : GADFullScreenContentDelegate {
    ...
    public func ad(_ ad: GADFullScreenPresentingAd, didFailToPresentFullScreenContentWithError error: Error) {
      print("Ad did fail to present full screen content.")
        self.adsDidFailToPresentFullScreenContentWithError(ad, error)
    }

    /// Tells the delegate that the ad will present full screen content.
    public func adWillPresentFullScreenContent(_ ad: GADFullScreenPresentingAd) {
        self.adsWillPresentFullScreenContent(ad)
    }

    /// Tells the delegate that the ad dismissed full screen content.
    public func adDidDismissFullScreenContent(_ ad: GADFullScreenPresentingAd) {
        self.adsDidDismissFullScreenContent(ad)
    }
}
