...
extension Reactive where Base: UIView {
    func tap() -> Observable<Void> {
        return tapGesture().when(.recognized).mapToVoid().throttle(.milliseconds(300), scheduler: MainScheduler.instance)
    }
    
    /**
     - parameters:
        - observables: List of Observable type Bool
     
     - returns: result event if all of `observables` results is True, else throw Error
     
     Example:
     ```
     meshNode.view.rx
         .tapWith(
             PingManager.shared.rxPing().map({ $0 == true})  // Gọi Ping network, Bắt buộc result == true
         )
         .subscribe(onNext: { [weak self] success in
             if success {
                 self?.viewModel?.openListMeshes()
             } else {
                 // Nếu Ping network không thành công => Show Toast lỗi
                 AppMessagesManager.shared.showMessage(messageType: .error, message: OHText.unknownError)
             }
         })
         .disposed(by: disposeBag)
     ```
     */
    func tapWith(_ observables: Observable<Bool> ...) -> Observable<Bool> {
        return tap()
        // Call APIs... after tap and wait for result
        // Disable interaction
            .flatMap { _ in
        ...
        // Enabled interaction
            .flatMap { results in
                ...
                let success = !results.contains(false)
    ...
    /**
     ...
     ```
     */
    func tapWithNetwork() -> Observable<Void> {
        return tapWith(
            PingManager.shared.rxPing().map({ $0 == true})  // Gọi Ping network, Bắt buộc result == true
        )
        .flatMap{ success in
            ...
            // Nếu Ping network không thành công => Show Toast lỗi
            AppMessagesManager.shared.showMessage(messageType: .error, message: "Unknow error")
