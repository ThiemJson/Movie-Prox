...
extension AppMessagesManager {
    ...
    func showCenterMessage(messageType type: Theme, withTitle title: String = "", message: String, isPresented: Bool? = nil,  completion: (() -> Void)? = nil, duration: SwiftMessages.Duration = .seconds(seconds: 4)) {
        ...
        if var topController = UIApplication.shared.keyWindow?.rootViewController {
            ...
            topController.view.endEditing(true)
            topController.view.hideToast()
            topController.view.makeToast(message, position: .center)
            completion?()
        }
    }
